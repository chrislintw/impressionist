...
describe Impressionist do
  let(:imp) { RUBY_VERSION =~ '1.8' ? 'is_impressionable' : :is_impressionable }
  ...
  it 'should include methods in ApplicationController' do
    method = RUBY_VERSION =~ '1.8' ? 'impressionist' : :impressionist
  ...
  it 'should include the before_filter method in ApplicationController' do
    ...
    filters.collect(&:filter).include?(:impressionist_app_filter).should be_true
  end
end
